...
public class AdminDAOImplement implements AdminDAO{
	...
	public boolean updateBook(BookBean book) {
		try{
			...
			BookBean record1 = manager.find(BookBean.class, book.getBId());
			record1.setAuthor(book.getAuthor());
			BookBean record2 = manager.find(BookBean.class, book.getBId());
			record2.setPublisher(book.getPublisher());
			BookBean record3 = manager.find(BookBean.class, book.getBId());
			record3.setCategory(book.getCategory());
			transaction.commit();
			return true;
		}catch (Exception e) {
			System.err.println(e.getMessage());
