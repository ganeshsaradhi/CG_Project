...
public class AdminDAOImp implements AdminDAO{
	...
	public boolean issueBook(int id , int book_id) {
		...
		try {
			...
			transaction = manager.getTransaction();
			transaction.begin();
			Query query = manager.createQuery("select b from BookBean b where b.bid = :bbid and b.copies>=1");
			Query books = query.setParameter("bbid", book_id);
			List count = books.getResultList();
			int bookCount = count.size();
			if(bookCount >= 1) {
				Query query1 = manager.createQuery("select r from RequestBook r where r.id = :id and r.reqBookPK.bid = :bid");
				 query1.setParameter("id", id);
				 query1.setParameter("bid", book_id);
				List count1 = query1.getResultList();
				int reqBook = count1.size();
				if(reqBook >= 1) {
					Query query2 = manager.createQuery("select count(id) as idCount from BorrowedBookBean b where id=:id");
					...
					if(borrowBook >= 1) {
						...
						if(noOfBooksBorrowed<3) {
							...
							Query query3 = manager.createNativeQuery("insert into bookissue1 (id,bid,email,issueDate,returnDate) values (? , ? , ? , ? , ?) ");
							...
							if(count3 != 0) {
								...
								Query query4 = manager.createNativeQuery("insert into borrowbook1 (id,bid,email) values (?,?,?)");
								query4.setParameter(1, id);
								query4.setParameter(2, book_id);
								query4.setParameter(3, userEmail1List.get(0));
								query4.executeUpdate();
							
							Query query5 = manager.createQuery("delete from RequestBook r where r.id = :id and r.reqBookPK.bid = :bid");
							query5.setParameter("id", id);
							query5.setParameter("bid", book_id);
							query5.executeUpdate();
							Query query6 = manager.createQuery("update BookBean b set b.copies = b.copies-1 where b.bid = :bid");
	...
	public boolean returnBook(int id, int book_id) {
		...
		try {
			...
			transaction = manager.getTransaction();
			transaction.begin();
			Query query = manager.createQuery("select b from BookIssueDetails b where b.id = :id and b.issuePK.bid = :bid");
			query.setParameter("id", id);
			query.setParameter("bid", book_id);
			List count = query.getResultList();
			int i = count.size();
			if(i >= 1) {
				Query query1 = manager.createQuery("select r from RequestBook r where r.id = :id and r.reqBookPK.bid = :bid and type = :type");
				query1.setParameter("id", id);
				query1.setParameter("bid", book_id);
				...
				int i1 = count1.size();
				if(i1 >= 1) {
					Query query2 = manager.createQuery("update BookBean b  set b.copies = b.copies+1 where b.bid = :bid");
